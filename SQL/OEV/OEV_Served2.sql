...
SELECT
    ...
    IF(club_q1 IS NOT NULL, 'yes', IF(quest_q1 IS NOT NULL, 'yes', IF(arv_1 IS NOT NULL, 'yes', IF(odk_1 IS NOT NULL, 'yes', IF(ptme_1 IS NOT NULL, 'yes', IF(ptme1_1 IS NOT NULL, 'yes', IF(mereenfant_1 IS NOT NULL, 'yes', IF(opf_q1.case_id IS NOT NULL, 'yes', IF(tmf_q1.patient_code IS NOT NULL, 'yes', IF(tpv_q1.patient_code IS NOT NULL, 'yes', IF(oto1.patient_code IS NOT NULL, 'yes', 'no'))))))))))) AS q1,
    IF(club_q2 IS NOT NULL, 'yes', IF(quest_q2 IS NOT NULL, 'yes', IF(arv_2 IS NOT NULL, 'yes', IF(odk_2 IS NOT NULL, 'yes', IF(ptme_2 IS NOT NULL, 'yes', IF(ptme1_2 IS NOT NULL, 'yes', IF(mereenfant_2 IS NOT NULL, 'yes', IF(opf_q2.case_id IS NOT NULL, 'yes', IF(tmf_q2.patient_code IS NOT NULL, 'yes', IF(oto2.patient_code IS NOT NULL, 'yes', IF(tpv_q2.patient_code IS NOT NULL, 'yes', 'no'))))))))))) AS q2,
    ...
    IF(COALESCE(ptme_1, ptme1_1) IS NOT NULL, 'yes', 'no') AS ptme_q1,
    IF(COALESCE(ptme_2, ptme1_2) IS NOT NULL, 'yes', 'no') AS ptme_q2,
    IF(mereenfant_1 IS NOT NULL, 'yes', 'no') AS mereenfant_q1,
    IF(mereenfant_2 IS NOT NULL, 'yes', 'no') AS mereenfant_q2,
    office,
...
FROM
    ...
    UNION
    SELECT
        topf.patient_code AS patient_code
    FROM
        tracking_odk_phone_followup topf
    WHERE
        topf.eccm_joignable_par_tel != 0
        AND topf.eccm_date BETWEEN @start_date_last AND @end_date
    ...
    WHERE
        tpv.date_of_visit BETWEEN @start_date_last AND @end_date
    UNION
    SELECT
        opt.patient_code
    FROM
        odk_tracking_other_visit_ptme opt
    WHERE
        opt.date_of_visit BETWEEN @start_date_last AND @end_date
    ) bot
LEFT JOIN
    (SELECT DISTINCT
    ...
    ) c2 ON club_q2 = bot.patient_code
LEFT JOIN patient ON patient.patient_code = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE
        is_present = 1
        AND club_session.date BETWEEN @start_date_last AND @end_date_last
    ) c1 ON club_q1 = bot.patient_code
LEFT JOIN
    (SELECT
    ...
    ) quest ON quest_q1 = bot.patient_code
LEFT JOIN
    (SELECT
    ...
    LEFT JOIN patient ON patient.id = y.id_patient
    ) ques ON quest_q2 = bot.patient_code
LEFT JOIN lookup_hospital ON CONCAT(lookup_hospital.city_code, '/', lookup_hospital.hospital_code) = LEFT(patient.patient_code, 8)
LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE
        ...
        AND category = 'regime_mother_treatment'
    ) ar ON arv_1 = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
...
LEFT JOIN
    ...
    ) odk1 ON odk_1 = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
        health_id AS odk_2
    FROM
    ...
    ) odk2 ON odk_2 = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
    ...
    WHERE
        ...
        OR (actual_delivery_date BETWEEN @start_date_last AND @end_date_last)
    ) ptme1 ON ptme_1 = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
    ...
    ) ptme2 ON ptme_2 = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
    ...
    LEFT JOIN patient ON patient.id = id_patient
    WHERE
        PTME_date BETWEEN @start_date_last AND @end_date_last
    ) pt1 ON ptme1_1 = bot.patient_code
LEFT JOIN
    (SELECT DISTINCT
    ...
    LEFT JOIN patient ON patient.id = id_patient
    WHERE
        PTME_date BETWEEN @start_date AND @end_date
    ) pt2 ON ptme1_2 = bot.patient_code
LEFT JOIN
    (SELECT
    ...
    ) me1 ON mereenfant_1 = bot.patient_code
LEFT JOIN
    (SELECT
    ...
    WHERE
        ...
        AND patient_code IS NOT NULL
    GROUP BY patient_code
    ) me2 ON mereenfant_2 = bot.patient_code
LEFT JOIN patient p ON p.patient_code = bot.patient_code
LEFT JOIN
    (SELECT
        case_id, eccm_date, topf.patient_code
    FROM
        tracking_odk_phone_followup topf
    WHERE
        topf.eccm_joignable_par_tel != 0
        AND topf.eccm_date BETWEEN @start_date_last AND @end_date_last
    GROUP BY topf.case_id
